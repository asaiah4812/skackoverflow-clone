...
def ask():
    ...
    if form.validate_on_submit():
        try:
            ...
            # Log success
            current_app.logger.info(f"Question created: ID={question.id}, Title='{question.title}'")
...
@questions_bp.route('/tag/<int:tag_id>')
def tag_questions(tag_id):
    """List questions with a specific tag."""
    tag = Tag.query.get_or_404(tag_id)
    page = request.args.get('page', 1, type=int)
    
    questions = Question.query.filter(Question.tags.any(Tag.id == tag_id))\
